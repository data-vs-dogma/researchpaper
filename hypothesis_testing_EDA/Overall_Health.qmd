...
```{r}
# Boxplot for health care quality and prevention by abortion policies
...
```{r}
# Boxplot for health outcomes by abortion policies
...
```{r}
# Create the OBGYN ratio column
df %>% filter(!is.na(number_of_obstetricians_and_gynecologists_employed_2023) &
         !is.na(total_number_of_women_aged_15_44_2017))
df <- df %>%
  mutate(obgyn_ratio = number_of_obstetricians_and_gynecologists_employed_2023 /
                       total_number_of_women_aged_15_44_2017)

# Summarize the ratio by abortion policy
ratio_summary <- df %>%
  group_by(abortion_policies) %>%
  summarise(mean_ratio = mean(obgyn_ratio, na.rm = TRUE),
            median_ratio = median(obgyn_ratio, na.rm = TRUE),
            sd_ratio = sd(obgyn_ratio, na.rm = TRUE))
# Print summary table
print(ratio_summary)
...
```{r}
ggplot(df, aes(x = abortion_policies, y = obgyn_ratio, fill = abortion_policies)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#1c7416", "#68bb59", "#acdf87", "#fab733", "#ff6242", "#ff0000", "#c61a09")) +
  labs(
    x = "Abortion Policies",
    y = "Ratio of OB-GYNs to Women (15-44)",
    title = "OB-GYN Ratios by Abortion Policy"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
for (i in 1:N) {
  permuted_data <- df
  permuted_data$obgyn_ratio <- sample(permuted_data$obgyn_ratio)
  
  perm_anova <- aov(obgyn_ratio ~ abortion_policies, data = permuted_data)
  perm_F[i] <- summary(perm_anova)[[1]][["F value"]][1]
}
...
# Calculate the overall average
overall_avg <- mean(df$obgyn_ratio, na.rm = TRUE)

# Filter and classify observations as Above or Below Average
df <- df %>%
  filter(!is.na(abortion_policies) & !is.na(obgyn_ratio)) %>% 
  mutate(above_or_below = if_else(obgyn_ratio > overall_avg, 
                                  "Above Average", "Below Average"))
