...
restrictive_policy_columns2 <- c(
  "state_constitution_excludes_abortion_rights", 
  "waiting_period_after_counseling", 
  "waiting_period_after_required_in_person_counseling", 
  "forced_ultrasound", 
  "state_medicaid_coverage_ban", 
...
plot_data_restrictions <- df %>%
  select(state, all_of(restrictive_policy_columns2), all_of(gestational_ban_vars)) %>%
  pivot_longer(
...
df_protectiveness <- df %>%
  mutate(Protectiveness_Score = rowSums(select(., all_of(protection_vars)), na.rm = TRUE)) %>%
...
plot_usmap(data = df_protectiveness, values = "Protectiveness_Score", regions = "states") +
  ...
  labs(title = "State Protectiveness Based on Abortion Protection Laws") +   theme_minimal() +
  theme(legend.position = "right",plot.title = element_text(hjust = 0.5, size = 14),
  legend.title = element_text(size = 12), legend.text = element_text(size = 10))
```
```{r}
...
```
* The model shows a statistically significant negative relationship between Restrictiveness_Score and Protectiveness_Score (β=−0.3555, p=0.0263). This indicates that as states become more restrictive in abortion policies, they tend to have fewer protective laws in place.

* The R2value is 0.07828, meaning approximately 7.8% of the variance in protectiveness is explained by restrictiveness. While this shows a weak predictive power, it still highlights an inverse correlation worth exploring further.

* The significant negative coefficient suggests a potential trade-off between restrictiveness and protectiveness in state policies, reinforcing the need to examine how restrictive policies may impede protective measures or reflect policy prioritization in states.
...
pheatmap(correlation_matrix,
         ...
         main = "Policy Clustering Based on Correlations",
         fontsize_row = 8,          
         fontsize_col = 8,          
         angle_col = 45,            
         treeheight_row = 50,       
