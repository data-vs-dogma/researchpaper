...
## Distribution of Abortion Ban Types by State
...
ggplot(ban_data_yes, aes(x = Policy, fill = Policy)) +
  geom_bar(alpha = 0.8) +
  labs(
    title = "Distribution of Abortion Policy",
    x = "Abortion Policy",
    y = "Number of States",
    fill = "Policy Type"
  ) +
...
```{r}
# Map of Abortion Ban Types by State 
...
```{r}
protection_vars <- c("protections_to_enter_a_clinic", "shield_law_protecting_providers",
                     "patient_reproductive_health_data_privacy")
...
df %>%
  ...
  geom_bar() +
  labs(title = "Protections and Access Laws Across States", 
       x = "Protection Laws", y = "Count") +
  theme_minimal() +
...
ggplot(policy_counts, aes(x = reorder(Policy, -Count), y = Count, fill = Policy)) +
  ...
  coord_flip() +
  labs(
    title = "Number of States Implementing Each Policy",
    x = "Policy",
    y = "Number of States"
  ) +
  theme_minimal() +
...
```{r}
# Group states by Medicaid expansion and total abortion ban
medicaid_policy <- df %>%
  group_by(medicaid_expansion_status, total_abortion_ban) %>%
  summarise(Count = n(), .groups = "drop") %>%
  mutate(total_abortion_ban = ifelse(total_abortion_ban == 1, "Yes", "No"),
         medicaid_expansion_status = ifelse(medicaid_expansion_status == 1, "Expanded", "Not Expanded"))

# Plot
ggplot(medicaid_policy, aes(x = medicaid_expansion_status, y = Count, fill = total_abortion_ban)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Medicaid Expansion vs. Total Abortion Ban",
    x = "Medicaid Expansion Status",
    y = "Number of States",
    fill = "Total Abortion Ban"
  ) +
  theme_minimal()
