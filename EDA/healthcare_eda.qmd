...
depression_screening <- create_policy_plot(df, "%_women_who_received_a_postpartum_depression_screening_2021",
...
depression_screening_scatter <- create_policy_scatter_plot(
  df,
  "%_women_who_received_a_postpartum_depression_screening_2021",
  "Women Receiving Postpartum Depression Screening (2021)",
  "Percentage"
)
...
# Prepare the data by filtering out NA values
df_clean <- df %>%
  filter(
    !is.na(`maternal_mortality_rates_2018-2022_per_100,000_live births`),
    !is.na(abortion_policies)
  ) %>%
  mutate(
...
# Define a color palette based on abortion policy
policy_colors <- c(
  "most restrictive" = "#8B0000",    # Dark red
  "very restrictive" = "#FF4500",    # Red
  "restrictive" = "#FF6347",         # Light red
  "some restrictions/protections" = "#FFD700",  # Yellow
  "protective" = "#98FB98",          # Light green
  "very protective" = "#32CD32",     # Green
  "most protective" = "#006400"      # Dark green
)

# Plot: Maternal Mortality Rates with Color-Coded Bars
ggplot(df_clean, 
       aes(x = reorder(State, `maternal_mortality_rates_2018-2022_per_100,000_live births`),
           y = `maternal_mortality_rates_2018-2022_per_100,000_live births`,
  ...
  geom_text(aes(label = sprintf("%.1f", `maternal_mortality_rates_2018-2022_per_100,000_live births`)),
            hjust = -0.1,
            size = 3.5) +
  scale_y_continuous(limits = c(0, max(df_clean$`maternal_mortality_rates_2018-2022_per_100,000_live births`, na.rm = TRUE) * 1.15))
