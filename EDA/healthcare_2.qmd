...
df_clean <- data.frame(
  State = c("Tennessee", "Mississippi", "Alabama", "Arkansas", "Louisiana", "Kentucky", "Virginia", 
            "South Carolina", "Georgia", "Indiana", "Arizona", "Oklahoma", "Texas", "New Mexico", 
            "North Carolina", "New Jersey", "Nebraska", "Ohio", "Florida", "West Virginia", "Missouri",
            "US Average", "Kansas", "New York", "Maryland", "Nevada", "Idaho", "Iowa", "Michigan",
            "Illinois", "Washington", "Pennsylvania", "Oregon", "Massachusetts", "Colorado", 
            "Connecticut", "Utah", "Wisconsin", "Minnesota", "California"),
  
  maternal_mortality = c(41.1, 39.1, 38.6, 38.3, 37.3, 34.6, 32.7, 32.3, 32.1, 30.9, 30.0, 
                        29.6, 28.2, 28.0, 26.7, 26.0, 25.1, 24.5, 24.1, 23.9, 23.8, 23.2, 
                        22.8, 22.4, 21.3, 20.4, 20.0, 19.5, 19.1, 18.1, 18.0, 17.5, 16.6, 
                        16.4, 16.0, 15.6, 15.5, 13.2, 12.3, 10.5),
  
  abortion_policies = c("most restrictive", "most restrictive", 
                       "very restrictive", "US Average", "restrictive", "very protective",
                       "protective", "most restrictive",
                       "most protective")
)
...
policy_order <- c("most protective", "very protective", "protective",
                 "some restrictions/protections", "restrictive",
                 "very restrictive", "most restrictive", "US Average")

# Convert abortion_policies to factor with specified order
df_clean$abortion_policies <- factor(df_clean$abortion_policies, levels = policy_order)

# Define colors for the abortion policies
policy_colors <- c(
  "most restrictive" = "#8B0000",     # Dark red
  "very restrictive" = "#FF4E50",     # Bright red
  "restrictive" = "#FFC857",          # Yellow
  "some restrictions/protections" = "#90EE90",  # Light green
  "protective" = "#98FB98",           # Pale green
  "very protective" = "#228B22",      # Forest green
  "most protective" = "#006400",      # Dark green
  "US Average" = "#1f77b4"           # Blue
)

# Create the plot
ggplot(df_clean, 
       aes(x = reorder(State, maternal_mortality),
  ...
  geom_text(
    aes(label = sprintf("%.1f", maternal_mortality)),
    hjust = -0.2, 
    size = 3,
    color = "black"
  ) +
  scale_y_continuous(
    limits = c(0, max(df_clean$maternal_mortality) * 1.15),
...
policy_colors <- c(
  "most restrictive" = "#8B0000",    
  "very restrictive" = "#FF4E50", 
  "restrictive" = "#FFC857",     
  "some restrictions/protections" = "#90EE90", 
  "protective" = "#98FB98",      
  "very protective" = "#228B22",  
  "most protective" = "#006400",     
  "US Average" = "#1f77b4"        
)
...
df_clean_filtered$abortion_policies <- factor(df_clean_filtered$abortion_policies, 
                                   levels = rev(policy_order))  

us_average <- 23.2

policy_colors <- c(
  "most restrictive" = "#8B0000",     
  "very restrictive" = "#FF6B6B",   
  "restrictive" = "#FFB347",  
  "some restrictions/protections" = "#FFD700",  
  "protective" = "#90EE90",          
  "very protective" = "#228B22", 
  "most protective" = "#006400"       
)
...
ggplot(df_clean_filtered, 
       aes(y = abortion_policies, 
  ...
  geom_vline(xintercept = us_average, 
             linetype = "dashed", 
             color = "#0066CC",        
             size = 0.5) +
