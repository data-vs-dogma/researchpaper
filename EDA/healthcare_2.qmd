...
# Coverage, Access, Accountability Permutation Test
...
# Healthcare Quality Prevention Permutation Test
...
# Health Outcomes Permutation Test
...
# Women who went without care because of cost Permutation Test
...
# Uninsured Women Permutation Test
...
# Uninsured Pregnancy Permutation Test
...
result <- permutation_test(
  data = df,
  group_col = "abortion_policies",
  value_col = "women_with_depression_before_or_during_pregnancy_2021"
)
...
###
Null hypothesis: Depression levels do not differ across abortion policy groups (i.e., all groups come from the same distribution).

Alternate hypothesis: There is a significant difference in depression between the abortion policy groups.

The p-value is 0.06831, there is moderate evidence against the null hypothesis but not strong enough to reject it at the 5% significance level.
...
###
In this case, the observed F-statistic lies close to, but not far enough away from the bulk of the null distribution, meaning it is not extreme enough to fall in the rejection region
...
# Depression Screenings Permutation Test
...
###
Null hypothesis: Post partum depression screening levels do not differ across abortion policy groups (i.e., all groups come from the same distribution).

Alternate hypothesis: There is a significant difference in post partum depression screening levels between the abortion policy groups.

The p-value is 0.0048, there is evidence to reject the null hypothesis.
...
result <- permutation_test(
  data = df,
  group_col = "abortion_policies",
  value_col = "percent_women_experiencing_intimate_partner_violence_2021"
)
...
###
Null hypothesis: Intimate partner violence does not differ across abortion policy groups (i.e., all groups come from the same distribution).

Alternate hypothesis: There is a significant difference in intimate partner violence between the abortion policy groups.

The p-value is 0.06947, there is not enough evidence to reject the null hypothesis.
...
###
I'd like to look at just the most restrictive states because they are significantly higher. 
...
## Maternal Mortality Permutation Test
...
The p-value is 0.00285, which provides provides evidence to reject the null hypothesis at a 5% signicance level.
